...
const xmlns = "http://www.sitemaps.org/schemaa/sitemap/0.9"
...
func main() {
	urlFlag := flag.String("url", "https://gophercises.com", "url to parse sitemap")
	maxDepth := flag.Int("depth", 10, "the maximum number of links deep to traverse")
	flag.Parse()
	pages := bfs(*urlFlag, *maxDepth)
	// fmt.Printf("parse sitemap from %v\n", *urlFlag)
	// pages := get(*urlFlag)
	toXml := urlset{
		Xmlns: xmlns,
	}
